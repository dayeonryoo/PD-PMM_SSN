...
bool SSN_PMM<T>::is_PSD(const SpMat& Q) {
    ...
    if (diag) { // If so, check diagonal elements.
        ...
        for (int i = 0; i < n; ++i) {
            L.insert(i, i) = std::sqrt(Q.coeff(i, i));
        }
        L_tr = L;
        return true;
    }
...
typename SSN_PMM<T>::Vec SSN_PMM<T>::compute_residual_norms() {
    ...
    // Dual residual norm
    T res_d = (c + Q * x - A.transpose() * y1.head(m) - B.transpose() * y2 + z).norm() / (1 + c.norm());
...
Solution<T> SSN_PMM<T>::solve() {
    ...
    while (PMM_iter < max_iter) {
        std::cout << "PMM main loop started!\n";
    // ----------------------------------------------
    // Structure:
    ...
    // ----------------------------------------------
        ...
        printer(PMM_iter, opt, obj_val, x, v, y1, y2, z, PMM_tol_achieved);
    ...
    return Solution<T>(opt, x, v, y1, y2, z, obj_val, PMM_iter, SSN_iter, PMM_tol_achieved, SSN_tol_achieved);
}
